        ...
        public void Insert_Facture(Facture facture)
        {
            ...
            using (SqlConnection connection = new SqlConnection(Connexion.ConnectionString))
            {
                ...
                try
                {
                    ...
                    {
                        SqlCommand cmd = new SqlCommand($"INSERT INTO Factures (Num_Fac, Fournisseur, Date_Fac, Date_Saisie,Designation,ID_FAC,[Order]) " +
                                                    $" VALUES('{facture.NumFacture}','{fournisseur.Trim().Replace("'", "''")}'," +
                                                    $"'{facture.Date_Facture.ToString("dd/MM/yyyy")}'," +
                                                    $"'{Convert.ToDateTime(facture.Date_Saisie.ToString("dd/MM/yyyy hh:mm:ss.fffffff"))}','{facture.Designation}','{facture.GuidVal}',{Order})", connection, transaction);
                        cmd.ExecuteNonQuery();
                    }
                    else
                    {
                        SqlCommand cmd = new SqlCommand($"INSERT INTO Factures (Num_Fac, Fournisseur, Date_Fac, Date_Saisie,Designation,ID_FAC,[Order],New_Fournisseur) " +
                                                    $" VALUES('{facture.NumFacture}','{fournisseur.Trim().Replace("'", "''")}'," +
                                                    $"'{facture.Date_Facture.ToString("dd/MM/yyyy")}'," +
                                                    $"'{Convert.ToDateTime(facture.Date_Saisie.ToString("dd/MM/yyyy hh:mm:ss.fffffff"))}','{facture.Designation}','{facture.GuidVal}',{Order},'{facture.New_Fournisseur.Trim()}')", connection, transaction);
                        cmd.ExecuteNonQuery();
                    }
